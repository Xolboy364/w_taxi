...
from states import ServiceAdStates, AdminRejectState
...
async def service_ad_start(message: Message, state: FSMContext):
    ...
    # Bitta foydalanuvchida bir vaqtda faqat 1 ta ko'rib chiqilayotgan e'lon bo'lsin (spam nazorati)
    if await db.has_pending_service_ad(message.from_user.id):
        await message.answer(
            "⏳ Sizda hozir ko‘rib chiqilayotgan e'lon bor. Admin javobini kuting yoki "
            "holatini <b>📋 Mening e'lonlarim</b> bo‘limidan tekshiring.",
            parse_mode="HTML"
        )
        return
...
async def my_service_ads(message: Message, state: FSMContext):
    if not await check_access_svc(message): return
    await state.clear()

    ads = await db.get_user_service_ads(message.from_user.id)
    if not ads:
    ...
    for ad in ads:
        svc_info = SERVICE_TYPES.get(ad["service_type"], {"name": ad["service_type"]})
        pay_status = ad["payment_status"]

        if pay_status == "pending":
            status_line = "⏳ Ko‘rib chiqilmoqda"
        elif pay_status == "rejected":
            reason = ad["reject_reason"] or "Sabab ko'rsatilmagan"
            status_line = f"❌ Rad etilgan (Sabab: {reason})"
        elif ad["is_active"]:
            expires = ad["expires_at"].strftime("%d.%m.%Y") if ad["expires_at"] else "-"
            status_line = f"✅ Faol (tugash sanasi: {expires})"
        else:
            status_line = "⌛ Muddati tugagan"

        text += f"🏷 {svc_info['name']} — <b>{ad['name']}</b>\n{status_line}\n━━━━━━━━━━━━━━━━━━\n"
...
@router.callback_query(F.data == "adfuel_back")
async def ad_fuel_back(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text("Xizmat turini tanlang:", reply_markup=get_service_types_kb())
    await state.set_state(ServiceAdStates.choose_type)
    await callback.answer()
...
async def service_approve(callback: CallbackQuery, bot: Bot):
    ...
    # Atomik tekshiruv: agar allaqachon boshqa admin tasdiqlagan/rad etgan bo'lsa, qayta ishlamaymiz
    ad = await db.activate_service_ad_once(ad_id, days=30)

    if ad is None:
        await callback.answer("⚠️ Bu e'lon allaqachon ko'rib chiqilgan!", show_alert=True)
        try:
...
async def service_reject_start(callback: CallbackQuery, state: FSMContext):
    if not (await db.is_admin(callback.from_user.id, ADMIN_ID) or callback.from_user.id == ADMIN_ID):
        await callback.answer("Huquqingiz yo'q!", show_alert=True)
        return

    ad_id = int(callback.data.replace("svc_reject_", ""))
    # E'tibor bering: sabab matnini qayta ishlash handlers.py da (admin_reject_reason_submit)
    # bitta umumiy joyda amalga oshiriladi - shu orqali ikkita router bir xil state'ni
    # bir-biridan "o'g'irlab" qolish muammosining oldi olinadi.
    await state.update_data(
        reject_target_type="service_ad",
        reject_target_id=ad_id,
        reject_msg_id=callback.message.message_id,
        reject_chat_id=callback.message.chat.id
    )
    await callback.message.answer("Rad etish from aiogram import Router, F, Bot
from aiogram.types import Message, CallbackQuery, ReplyKeyboardRemove, InlineKeyboardMarkup, InlineKeyboardButton
from aiogram.fsm.context import FSMContext
...
async def service_reject_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
sababini yozing (foydalanuvchiga shu matn yuboriladi):")
    await state.set_state(AdminRejectState.enter_reason)
    await callback.answer()
