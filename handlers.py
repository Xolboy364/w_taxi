...
async def secret_admin_password_check(message: Message, state: FSMContext):
    curr_pass = await db.get_setting("super_admin_password", "admin777")
    if message.text.strip() == curr_pass:
        TEMP_SESSIONS[message.from_user.id] = time.time()
        await state.clear()
        await db.log_activity(message.from_user.id, message.from_user.full_name or "Noma'lum", "TEMP_SUPER_ADMIN_LOGIN", "Begona qurilmadan kirildi", is_temp_admin=1)
...
async def mon_start_2_cb(callback: CallbackQuery, bot: Bot):
    ...
    await callback.message.edit_text("💎 <b>Monetizatsiya muvaffaqiyatli yoqildi! Haydovchilarga xabar yuborilmoqda...</b>", parse_mode="HTML")
    ...
    driver_ids = await db.get_all_driver_ids()
    for did in driver_ids:
        try:
...
async def driver_receipt_received(message: Message, state: FSMContext, bot: Bot):
    ...
    approve_kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="✅ Obunani tasdiqlash (+30 kun)", callback_data=f"app_sub_{message.from_user.id}")]
        ]
    )
    caption = (
...
async def admin_approve_subscription(callback: CallbackQuery, bot: Bot):
    ...
    await db.extend_driver_subscription(target_id, days=30)
...
async def admin_manual_backup(message: Message, state: FSMContext):
    if not (is_super_admin(message.from_user.id) or await db.is_admin(message.from_user.id, ADMIN_ID)): return
    await state.clear()
    if not os.path.exists(db.DB_NAME): return
    await message.answer_document(document=FSInputFile(db.DB_NAME, filename="bot_backup.db"))
...
@router.message(F.location, F.text.in_(["🍽 Ovqatlanish", "🛏 Hostel va Mehmonxona", "🔧 Avtoservis"]))
